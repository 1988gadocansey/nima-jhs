...
//                    }
               ...
               if(empty($sql)){
                ...
                if ($feeLedger->save()) {
                   ...
                   @$firstName=@$balance[0]->FIRSTNAME;

                    $billOwing = (@$balance[0]->BILL_OWING - $amount)+$previousOwing;
                    if($level!="" &&$year!=""){
                        if($request->input('stno')!=""){
                    StudentModel::where('STNO', $indexno)->update(array('BILL_OWING' => $billOwing, 'TELEPHONENO' => $phone,'SYSUPDATE'=>'1'));
                        }
                        else{
                            StudentModel::where('INDEXNO', $indexno)->update(array('BILL_OWING' => $billOwing, 'TELEPHONENO' => $phone, 'LEVEL' => $level, 'YEAR' => $newyear,'SYSUPDATE'=>'1'));
                        ...
                        StudentModel::where('STNO', $indexno)->update(array('BILL_OWING' => $billOwing, 'TELEPHONENO' => $phone,'SYSUPDATE'=>'1'));
                             ...
                             StudentModel::where('INDEXNO', $indexno)->update(array('BILL_OWING' => $billOwing, 'TELEPHONENO' => $phone,'SYSUPDATE'=>'1'));
                        
                         }
                    }
                    if($request->input('stno')!=""){
                    $smsOwing = @StudentModel::where("STNO", $indexno)->where('STATUS', '=', 'In school')->get();
                        }
                    else{
                        $smsOwing = @StudentModel::where("INDEXNO", $indexno)->where('STATUS', '=', 'In school')->get();
                  ...
                  \Session::put('applicant', $indexno);
                    $message = "Hi $firstName, GHS$amount paid as $feetype, you owe GHS$smsOwe Please visit portal.tpolyonline.com to do your course registration. You can print your receipt using receipt no. $receipt ";
                    \DB::commit();
                    if ($sys->firesms($message, $phone, $indexno)) {
    ...
    public function printreceiptLate(Request $request, $receiptno) {
        ...
        if (empty($transaction)) {
            abort(434, "No Fee payment   with this receipt <span class='uk-text-bold uk-text-large'>{{$receiptno}}</span>");
                     ...
                     $data=  StudentModel::where("INDEXNO", $transaction->INDEXNO)->first();
