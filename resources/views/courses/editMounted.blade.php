   ...
   <div class="uk-overflow-container" id='print'>
      ...
      <tbody> <?php $count=0;?>
                                         ...
                                         @foreach($data as $index=> $row) 
                                           <input type="hidden" name="key" value="{{$ID}}" id="" />
                            
                                          <?php $count++;?>
                                        <tr align="">
                                            <td> {{ $data->perPage()*($data->currentPage()-1)+($index+1) }} </td>
                                              <td>
<!--                                                   <select placeholder='select courses' class="form-control" style="" name="course[]" required="required" class= 'md-input'v-model='course' v-form-ctrl='' v-select=''>
                                                    
                                                    @foreach($course as $item=>$rows)

                                                   <option <?php
                                                                            if ($rows->ID==$row->COURSE) {
                                                                                echo "selected='selected'";
                                                                            }
                                                                            ?> value="{{$rows->ID}}">{{$rows->COURSE_NAME}} -{{ $rows->COURSE_CODE }} - {{ $rows->COURSE_LEVEL }}yr</option>
                                                 @endforeach
                                                    </select>-->
                                            ...
                                            <td> {{$sys->getProgram($row->PROGRAMME)}} </td>
                                             ...
                                             <td>
                                                  ...
                                                  <select placeholder='select year' class="form-control" style="" name="level[]" required="required" class= 'md-input'v-model='course' v-form-ctrl='' v-select=''>
                                                      <option  <?php
                                                                            if ($row->COURSE_LEVEL =="1") {
                                                                                echo "selected='selected'";
                                                                            }
                                                                            ?>   value="1">1</option>
                                                    <option  <?php
                                                                            if ($row->COURSE_LEVEL =="2") {
                                                                                echo "selected='selected'";
                                                                            }
                                                                            ?>   value="2">2</option>

                                                   <option  <?php
                                                                            if ($row->COURSE_LEVEL =="3") {
                                                                                echo "selected='selected'";
                                                                            }
                                                                            ?>   value="3">3</option>
                                                  <option <?php
                                                                            ...
                                                                            ?> value="4">400</option>
                                                   <option<?php
                                                                            if ($row->COURSE_LEVEL =="400/1") {
                                                                                echo "selected='selected'";
                                                                            }
                                                                            ?>  value="400/1">400/1</option>
                                                    <option value="400/2" <?php
                                                                            if ($row->COURSE_LEVEL =="400/2") {
                                                                                echo "selected='selected'";
                                                                            }
                                                                            ?> >400/2</option>
