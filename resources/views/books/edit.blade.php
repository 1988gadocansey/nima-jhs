...
<h5 class="heading_b uk-margin-bottom">Edit Books   here</h5>
      ...
      <section id="payment_section" role="tabpanel" aria-labelledby="payment section" class="body step-1 "  v-bind:class="{'uk-hidden': !in_payment_section} "  data-step="1"  aria-hidden="true">
       ...
       <div data-uk-grid-margin="" class="uk-grid uk-grid-width-medium-1-4 uk-grid-width-large-1-4">
         ...
         <div class="parsley-row">
            ...
            <input type="hidden" value="{{$data->book_id}}" name='id'/>
...
var vm = new Vue({
 ...
 data : {
  ...
  publication:"{{$data->date_published}}",
   place:"{{$data->place_of_publication}}",
    publisher:"{{$data->book_pub}}",
     copies:"{{$data->book_copies}}",
   accention:"{{$data->accention_no}}",
   isbn:"{{$data->isbn}}",
   copyright:"{{$data->copyright_year}}",
    received:"{{$data->date_receive}}",
   added:"{{$data->date_added}}",
  collation:"{{$data->collation}}",
 category:"{{$data->category}}",
class:"{{$data->class_mark}}",
format:"{{$data->format}}",
series:"{{$data->series}}",
...
<script>
            ...
            var year = $('.year').val();
                    ...
                    UIkit.modal.confirm("Are you sure every data is accurate?? "
                            , function(){
                            modal = UIkit.modal.blockUI("<div class='uk-text-center'>Updating data <br/><img class='uk-thumbnail uk-margin-top' src='{!! url('public/assets/img/spinners/spinner.gif')  !!}' /></div>");
